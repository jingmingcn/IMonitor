...
die `pod2text $0` unless(@ARGV==9 || @ARGV==10);


my ($cope_f, $java_f, $log, $Q, $rate , $out , $out_id , $out_insert,$cal_qual_v, $single_read);
if(@ARGV==9){
	($cope_f, $java_f, $log, $Q, $rate , $out , $out_id , $out_insert,$cal_qual_v) = @ARGV;
}else{
	($cope_f, $java_f, $log, $Q, $rate , $out , $out_id , $out_insert,$cal_qual_v, $single_read) = @ARGV;
}

if(defined($single_read)){
	open COM, "|gzip >$java_f.gz" or die;
}
...
JAVA:while(<I>)
{
	...
	chomp;
        ...
        chomp(my $qual = <I>);
	if(defined($single_read)){
		my ($id1,$id2) = split /__/,$id;
		$further_merged_ids{$id1} = 1;
		$further_merged_ids{$id2} = 1;
		print COM "$id\n$seq\n$f\n$qual\n";
	}
...
JAVA:while(<I>)
{
        ...
        chomp(my $f = <I>);
        chomp(my $qual = <I>);
	next if(exists $further_merged_ids{$id});
	$single_read_n++;
	print COM "$id\n$seq\n$f\n$qual\n";
        # 1.1 insert size length
        my $len = length($seq);
...
if(defined($single_read)){
	print "\tSingle_reads: $single_read_n\t",$single_read_n/$total*100,"\n";
	close COM;
}
