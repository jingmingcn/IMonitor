...
=head1 Usage
	perl Merge_filter_fqtofa.pl  <java.merge.fq.gz> <filter-Q> <filter-rate> <out.fa.gz> <change_id.back.gz> <insert-size.distribution.gz><quality_system>
...
die `pod2text $0` unless(@ARGV==7);


my ( $java_f, $Q, $rate , $out , $out_id , $out_insert,$cal_qual_v) = @ARGV;
...
my ($total, $cope_n , $cope_r) = ($java_meger_n , 0 , 0);
