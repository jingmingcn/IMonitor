...
my ($non_func_f, $cdr3_len_f, $aa_cfw_f);
...
if($mut_f){
	...
	$v_m_r = $v_mut_all/$v_base_all*100;
	$v_s_r = $v_seq_mut/$vj_seq_all*100;

	$d_m_r = $d_mut_all/$d_base_all*100 if($d_base_all);
	$d_s_r = $d_seq_mut/$d_seq_all*100 if($d_seq_all);

	$j_m_r = $j_mut_all/$j_base_all*100;
	$j_s_r = $j_seq_mut/$vj_seq_all*100;

	$b_m_r = $mut_b_n/$all_b_n*100;
	$s_m_r = $seq_mut_n/$vj_seq_all*100;
...
my $r = sprintf("%0.2f",$flag_all{'in-frame'}/$tmp_all*100);
print "in-frame: $flag_all{'in-frame'}\t$r\n";
$r=  sprintf("%0.2f",$flag_all{'out-of-frame(stop_codon)'}/$tmp_all*100);
print "out-of-frame(stop_codon): $flag_all{'out-of-frame(stop_codon)'}\t$r\n";
$r=  sprintf("%0.2f",$flag_all{'out-of-frame(CDR3_length)'}/$tmp_all*100);
print "out-of-frame(CDR3_length): $flag_all{'out-of-frame(CDR3_length)'}\t$r\n";
$r=  sprintf("%0.2f",$flag_all{'non-function'}/$tmp_all*100);
